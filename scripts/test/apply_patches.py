...
iterating on patches without paying the cost of a full build.
    ...
    python -m scripts.test.apply_patches --device Amlogic-ng
    python -m scripts.test.apply_patches --device Amlogic-ng --revert
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    patches = lakka._collect_patches(REPO_ROOT, args.device, profile.project,
                                     profile.arch)
    if not patches:
